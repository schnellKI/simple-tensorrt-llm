...
def get_input_scalar_by_name(request, name, expected_batch_size=1, batch_index=0):
    ...
    if tensor.size != expected_batch_size:
        raise pb_utils.TritonModelException(f"Expected a scalar tensor for tensor {name}")
...
def get_parameter(model_config, name, pytype=str):
    if name not in model_config["parameters"]:
        return None
    return read_parameter_as_type(model_config["parameters"][name]["string_value"], name, pytype)
...
def convert_word_list(word_list):
    ...
    if len(word_list) == 0 or len(word_list[0]) != 2:
        raise pb_utils.TritonModelException("Invalid format for word list.")
...
def parse_medusa_choices(medusa_choices):
    if medusa_choices is None:
        return None
    try:
        result = json.loads("[" + medusa_choices.replace("{", "[").replace("}", "]") + "]")
...
def get_sampling_config_from_request(request, batch_size=1, batch_index=0):
    kwargs = {}
    kwargs["beam_width"] = (
        get_input_scalar_by_name(request, "beam_width", batch_size, batch_index) or 1
    )
    kwargs["top_k"] = get_input_scalar_by_name(request, "runtime_top_k", batch_size, batch_index)
    kwargs["top_p"] = get_input_scalar_by_name(request, "runtime_top_p", batch_size, batch_index)
    kwargs["top_p"] = None if kwargs["top_p"] is None or kwargs["top_p"] <= 0 else kwargs["top_p"]
    kwargs["random_seed"] = get_input_scalar_by_name(
        request, "random_seed", batch_size, batch_index
    )
    kwargs["temperature"] = get_input_scalar_by_name(
        request, "temperature", batch_size, batch_index
    )
    kwargs["min_length"] = get_input_scalar_by_name(request, "min_length", batch_size, batch_index)
    ...
    kwargs["top_p_min"] = get_input_scalar_by_name(
        request, "runtime_top_p_min", batch_size, batch_index
    )
    kwargs["top_p_reset_ids"] = get_input_scalar_by_name(
        request, "runtime_top_p_reset_ids", batch_size, batch_index
    )
    kwargs["top_p_decay"] = get_input_scalar_by_name(
        request, "runtime_top_p_decay", batch_size, batch_index
    )
    ...
    kwargs["num_return_sequences"] = (
        get_input_scalar_by_name(request, "num_return_sequences", batch_size, batch_index) or 1
    )
...
def get_output_config_from_request(request, batch_size=1, batch_index=0):
    ...
    if get_input_scalar_by_name(request, "return_kv_cache_reuse_stats", batch_size, batch_index):
...
def get_external_draft_tokens_config_from_request(request, batch_size=1, batch_index=0):
    kwargs = {}
    draft_input_ids = get_input_tensor_by_name(request, "draft_input_ids", batch_size, batch_index)
    if draft_input_ids is not None:
        kwargs["tokens"] = draft_input_ids[0].tolist()
    draft_logits = get_input_tensor_by_name(request, "draft_logits", batch_size, batch_index)
...
def get_prompt_tuning_config_from_request(request, batch_size=1, batch_index=0, input_length=0):
    ...
    if prompt_embedding_table is not None:
        if isinstance(prompt_embedding_table, np.ndarray):
            kwargs["embedding_table"] = from_numpy(prompt_embedding_table).squeeze(dim=0)
        ...
        if prompt_table_extra_ids is not None:
            ...
            if len(prompt_table_extra_ids) != 0:
                kwargs["input_token_extra_ids"] = prompt_table_extra_ids[0:input_length]
    kwargs = {k: v for k, v in kwargs.items() if v is not None}
    if len(kwargs) > 0:
...
def get_lora_config_from_request(request, batch_size=1, batch_index=0):
    kwargs = {}
    kwargs["task_id"] = get_input_scalar_by_name(request, "lora_task_id", batch_size, batch_index)
    lora_weights = get_input_tensor_by_name(request, "lora_weights", batch_size, batch_index)
    if lora_weights is not None:
        kwargs["weights"] = from_numpy(lora_weights).squeeze(dim=0)
    lora_config = get_input_tensor_by_name(request, "lora_config", batch_size, batch_index)
...
def get_guided_decoding_params_from_request(request, batch_size=1, batch_index=0):
    kwargs = {}
    ...
    if guided_decoding_guide_type is not None:
        guided_decoding_guide_type = guided_decoding_guide_type.squeeze(axis=0)[0].decode()
    ...
    guided_decoding_guide = get_input_tensor_by_name(
        request, "guided_decoding_guide", batch_size, batch_index
    )
...
def get_kv_cache_retention_config_from_request(request, batch_size=1, batch_index=0):
    def get_tensor_and_check_length(name: str, expected_length: int):
        tensor = get_input_tensor_by_name(request, name, batch_size, batch_index)
    ...
    if token_range_starts is not None:
        ...
        token_range_ends = [None if end == -1 else end for end in token_range_ends]
        ...
        token_range_durations_ms = get_input_tensor_by_name(
            request, "retention_token_range_durations_ms", batch_size, batch_index
        )

        if token_range_durations_ms is None:
            token_range_durations_ms = [None] * len(token_range_starts)
        else:
            token_range_durations_ms = np.squeeze(token_range_durations_ms, axis=0)
            token_range_durations_ms = [
                None if duration == -1 else duration for duration in token_range_durations_ms
            ]
        ...
        for start, end, priority, duration_ms in zip(
            token_range_starts, token_range_ends, token_range_priorities, token_range_durations_ms, strict=False
        ):
            ranges.append(
                trtllm.KvCacheRetentionConfig.TokenRangeRetentionConfig(
                    ...
                    else datetime.timedelta(milliseconds=duration_ms.item()),
                )
            )
        ...
        if decode_duration_ms is not None:
            decode_args["decode_duration_ms"] = (
                decode_duration_ms if decode_duration_ms != -1 else None
            )

        return trtllm.KvCacheRetentionConfig(token_range_retention_configs=ranges, **decode_args)

    return None


def get_lookahead_decoding_config_from_request(
    request, executor_lookahead_config, batch_size=1, batch_index=0
):
    lookahead_window_size = get_input_tensor_by_name(
        request, "lookahead_window_size", batch_size, batch_index
    )

    lookahead_ngram_size = get_input_tensor_by_name(
        request, "lookahead_ngram_size", batch_size, batch_index
    )
    ...
    if all(
        x is None
        for x in [lookahead_window_size, lookahead_ngram_size, lookahead_verification_set_size]
    ):
        return None
    ...
    return trtllm.LookaheadDecodingConfig(
        lookahead_window_size, lookahead_ngram_size, lookahead_verification_set_size
    )


def get_mrope_config_from_request(request, batch_size=1, batch_index=0):
    mrope_rotary_cos_sin = get_input_tensor_by_name(
        request, "mrope_rotary_cos_sin", batch_size, batch_index
    )
    mrope_position_deltas = get_input_tensor_by_name(
        request, "mrope_position_deltas", batch_size, batch_index, force_on_torch=False
    )
    assert (mrope_rotary_cos_sin is None) == (mrope_position_deltas is None), (
...
def convert_request(request, exclude_input_from_output, decoupled, executor_lookahead_config=None):
    ...
    if len(input_token_ids.shape) != 2:
        raise pb_utils.TritonModelException("Invalid format for input_ids")
    ...
    for batch_index in range(batch_size):
        ...
        input_length = get_input_scalar_by_name(request, "input_lengths", batch_size, batch_index)
        ...
        if inputs["max_new_tokens"] is None:
            raise pb_utils.TritonModelException("A value is required for request_output_len")
        inputs["streaming"] = get_input_scalar_by_name(
            request, "streaming", batch_size, batch_index
        )
        if inputs["streaming"] and not decoupled:
            raise pb_utils.TritonModelException("Streaming is only supported in decoupled mode.")

        inputs["end_id"] = get_input_scalar_by_name(request, "end_id", batch_size, batch_index)
        inputs["pad_id"] = get_input_scalar_by_name(request, "pad_id", batch_size, batch_index)
        inputs["stop_words"] = convert_word_list(
            get_input_tensor_by_name(request, "stop_words_list", batch_size, batch_index)
        )
        inputs["bad_words"] = convert_word_list(
            get_input_tensor_by_name(request, "bad_words_list", batch_size, batch_index)
        )
        embedding_bias = get_input_tensor_by_name(
            request, "embedding_bias", batch_size, batch_index
        )
        if embedding_bias is not None and embedding_bias.size != 0:
            inputs["embedding_bias"] = from_numpy(embedding_bias).squeeze(dim=0)

        sampling_config = get_sampling_config_from_request(request, batch_size, batch_index)
        output_config = get_output_config_from_request(request, batch_size, batch_index)
        ...
        if req_exclude_input_from_output is None:
            ...
            output_config.exclude_input_from_output = (
                exclude_input_from_output if exclude_input_from_output is not None else False
            )
        else:
        ...
        mrope_config = get_mrope_config_from_request(request, batch_size, batch_index)
        lora_config = get_lora_config_from_request(request, batch_size, batch_index)
        ...
        if encoder_input_features is not None:
            if isinstance(encoder_input_features, np.ndarray):
                encoder_input_features = from_numpy(encoder_input_features).squeeze(dim=0)
            ...
            logger.debug(f"inputs to llm: encoder_input_features ({encoder_input_features.shape}")
            ...
            if encoder_output_length is not None:
                inputs["encoder_output_length"] = np.squeeze(encoder_output_length, axis=0)
            ...
            if cross_attention_mask is not None:
                ...
                logger.debug(f"inputs to llm: cross_attention_mask ({cross_attention_mask.shape})")

            skip_cross_attn_blocks = get_input_tensor_by_name(
                request, "skip_cross_attn_blocks", batch_size, batch_index, force_on_torch=True
            )
...
def convert_response(
    response, batch_index, batch_size, num_return_sequences, expected_logits_dtype=torch.float32
):
    ...
    output_ids = np.full((1, len(result.output_token_ids), max_beam_length), -1, np.int32)
    ...
    if result.cum_log_probs is not None:
        output_tensors.append(
            pb_utils.Tensor(
                "cum_log_probs", np.expand_dims(np.array(result.cum_log_probs, np.float32), 0)
            )
        )
    ...
    if result.log_probs is not None:
        output_tensors.append(
            pb_utils.Tensor(
                "output_log_probs", np.expand_dims(np.array(result.log_probs, np.float32), 0)
            )
        )
    ...
    if result.generation_logits is not None:
        ...
        output_tensors.append(
            pb_utils.Tensor(
                ...
                np.expand_dims(
                    np.array(
                        result.generation_logits,
                        torch_to_numpy_dtype_dict[result.generation_logits.dtype],
                    ),
                    0,
                ),
            )
    ...
    if batch_size > 1:
        output_tensors.append(
            pb_utils.Tensor("batch_index", np.expand_dims(np.array([batch_index], np.int32), 0))
        )

    if num_return_sequences > 1:
        output_tensors.append(
            pb_utils.Tensor(
                "sequence_index", np.expand_dims(np.array([result.sequence_index], np.int32), 0)
            )
        )
    ...
    if result.request_perf_metrics is not None:
        ...
        output_tensors.append(
            pb_utils.Tensor(
                ...
                np.expand_dims(np.array([kv_cache_metrics.num_new_allocated_blocks], np.int32), 0),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                "kv_cache_reused_blocks",
                np.expand_dims(np.array([kv_cache_metrics.num_reused_blocks], np.int32), 0),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                ...
                np.expand_dims(
                    np.array([kv_cache_metrics.num_total_allocated_blocks], np.int32), 0
                ),
            )
        )
        ...
        output_tensors.append(
            pb_utils.Tensor(
                "arrival_time_ns",
                np.expand_dims(
                    np.array([pd.Timedelta(timing_metrics.arrival_time).value], np.int64), 0
                ),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                ...
                np.expand_dims(
                    np.array([pd.Timedelta(timing_metrics.first_scheduled_time).value], np.int64), 0
                ),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                "first_token_time_ns",
                np.expand_dims(
                    np.array([pd.Timedelta(timing_metrics.first_token_time).value], np.int64), 0
                ),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                "last_token_time_ns",
                np.expand_dims(
                    np.array([pd.Timedelta(timing_metrics.last_token_time).value], np.int64), 0
                ),
            )
        )
        ...
        output_tensors.append(
            pb_utils.Tensor(
                "acceptance_rate",
                np.expand_dims(np.array([spec_dec_metrics.acceptance_rate], np.float32), 0),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                ...
                np.expand_dims(
                    np.array([spec_dec_metrics.total_accepted_draft_tokens], np.int32), 0
                ),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                "total_draft_tokens",
                np.expand_dims(np.array([spec_dec_metrics.total_draft_tokens], np.int32), 0),
            )
        )

    return pb_utils.InferenceResponse(output_tensors), result.is_final, output_lengths
...
def convert_scheduler_policy(batch_scheduler_policy: str):
    ...
    if batch_scheduler_policy.lower() == "guaranteed_no_evict":
...
def convert_timestamp_to_seconds(timestamp: str):
    return int(datetime.datetime.strptime(timestamp, "%m-%d-%Y %H:%M:%S.%f").timestamp())
...
class TritonPythonModel:
    ...
    def get_kv_cache_config(self, model_config):
        kwargs = {
            "enable_block_reuse": get_parameter(model_config, "enable_kv_cache_reuse", bool),
            "max_tokens": get_parameter(model_config, "max_tokens_in_paged_kv_cache", int),
            ...
            "cross_kv_cache_fraction": get_parameter(
                model_config, "cross_kv_cache_fraction", float
            ),
            "host_cache_size": get_parameter(model_config, "kv_cache_host_memory_bytes", int),
            "onboard_blocks": get_parameter(model_config, "kv_cache_onboard_blocks", bool),
        }
        max_attention_window_size = get_parameter(model_config, "max_attention_window_size")
        if max_attention_window_size:
            kwargs["max_attention_window"] = [int(x) for x in max_attention_window_size.split(",")]
        kwargs = {k: v for k, v in kwargs.items() if v is not None}
    ...
    def get_parallel_config(self, model_config):
        ...
        if self.use_orchestrator_mode:
            ...
            spawn_processes = os.environ.get("TRTLLM_ORCHESTRATOR_SPAWN_PROCESSES", "1") == "1"
    ...
    def get_peft_cache_config(self, model_config):
        kwargs = {
            ...
            "max_adapter_size": get_parameter(model_config, "lora_cache_max_adapter_size", int),
            "device_cache_percent": get_parameter(
                model_config, "lora_cache_gpu_memory_fraction", float
            ),
            "host_cache_size": get_parameter(model_config, "lora_cache_host_memory_bytes", int),
            "lora_prefetch_dir": get_parameter(model_config, "lora_prefetch_dir", int),
        }
        kwargs = {k: v for k, v in kwargs.items() if v is not None}
        return trtllm.PeftCacheConfig(**kwargs)

    def get_executor_lookahead_config(self, model_config):
        lookahead_window_size = get_parameter(model_config, "lookahead_window_size", int)
        lookahead_ngram_size = get_parameter(model_config, "lookahead_ngram_size", int)
    ...
    def get_decoding_config(self, model_config):
        decoding_mode = convert_decoding_mode(get_parameter(model_config, "decoding_mode"))
        self.executor_lookahead_config = None
        if decoding_mode == trtllm.DecodingMode.Lookahead():
            # Add LAD config
            self.executor_lookahead_config = self.get_executor_lookahead_config(model_config)
        eagle_choices = parse_eagle_choices(get_parameter(model_config, "eagle_choices"))
        kwargs = {
            "medusa_choices": parse_medusa_choices(get_parameter(model_config, "medusa_choices")),
            "eagle_config": None if eagle_choices is None else trtllm.EagleConfig(eagle_choices),
            "lookahead_decoding_config": self.executor_lookahead_config,
    ...
    def get_extended_runtime_perf_knob_config(self, model_config):
        kwargs = {
            "multi_block_mode": get_parameter(model_config, "multi_block_mode", bool),
            "enable_context_fmha_fp32_acc": get_parameter(
                model_config, "enable_context_fmha_fp32_acc", bool
            ),
            "cuda_graph_mode": get_parameter(model_config, "cuda_graph_mode", bool),
            "cuda_graph_cache_size": get_parameter(model_config, "cuda_graph_cache_size", int),
        }
        kwargs = {k: v for k, v in kwargs.items() if v is not None}
        return trtllm.ExtendedRuntimePerfKnobConfig(**kwargs)

    def get_guided_decoding_config(self, model_config):
        guided_decoding_backend = get_parameter(model_config, "guided_decoding_backend", str)

        tokenizer_dir = get_parameter(model_config, "tokenizer_dir", str)
        if guided_decoding_backend not in ["xgrammar"]:
            if tokenizer_dir:
                pb_utils.Logger.log_warn(
                    "Guided decoding backend has not been set but tokenizer_dir is given. Tokenizer_dir will be ignored."
                )
            return None
        ...
        return trtllm.GuidedDecodingConfig(
            backend=guided_decoding_backend, **_xgrammar_tokenizer_info(tokenizer)
        )
    ...
    def get_executor_config(self, model_config):
        kwargs = {
            ...
            "enable_chunked_context": get_parameter(model_config, "enable_chunked_context", bool),
            "normalize_log_probs": get_parameter(model_config, "normalize_log_probs", bool),
            "batching_type": convert_batching_type(get_parameter(model_config, "gpt_model_type")),
            ...
            "max_queue_size": model_config.get(
                "dynamic_batching",
                {},
            )
            .get(
            ...
            .get("max_queue_size"),
            "extended_runtime_perf_knob_config": self.get_extended_runtime_perf_knob_config(
                model_config
            ),
            "guided_decoding_config": self.get_guided_decoding_config(model_config),
        }
        kwargs = {k: v for k, v in kwargs.items() if v is not None}
        return trtllm.ExecutorConfig(**kwargs)

    def create_metrics(self, model: str, version: str, is_v1_model: bool) -> None:
        ...
        self.all_metrics = {
            ...
            "num_waiting_requests": self.request_metric_family.Metric(
                labels={"request_type": "waiting", **common_labels}
            ),
            # Runtime metrics
            ...
            "fraction_used_blocks": self.kv_cache_metric_family.Metric(
                labels={"kv_cache_block_type": "fraction", **common_labels}
            ),
            # General metrics
            ...
            METRIC_TOTAL_OUTPUT_TOKENS: self.response_tokens_metric_family.Metric(
                labels={"response_metric_type": METRIC_TOTAL_OUTPUT_TOKENS, **common_labels},
                buckets=build_1_2_5_buckets(1000),
            ),
            METRIC_TOTAL_INPUT_TOKENS: self.request_tokens_metric_family.Metric(
                labels={"response_metric_type": METRIC_TOTAL_INPUT_TOKENS, **common_labels},
                buckets=build_1_2_5_buckets(1000),
            ),
        }
        if is_v1_model:
            self.all_metrics.update(
                {
                    "num_ctx_tokens": self.model_type_metric_family.Metric(
                        labels={"v1_specific_metric": "total_context_tokens", **common_labels}
                    ),
                    "num_gen_tokens": self.model_type_metric_family.Metric(
                        labels={"v1_specific_metric": "total_generation_tokens", **common_labels}
                    ),
                    "empty_gen_slots": self.model_type_metric_family.Metric(
                        labels={"v1_specific_metric": "empty_generation_slots", **common_labels}
                    ),
                }
            )
        else:
    ...
    def initialize(self, args) -> None:
        ...
        if get_parameter(model_config, "enable_trt_overlap", bool):
            raise pb_utils.TritonModelException("enable_trt_overlap=true is not supported.")
        ...
        for output in model_config["output"]:
            if output["name"] == "context_logits" or output["name"] == "generation_logits":
    ...
    def handle_stop_request(self, triton_user_id, response_sender) -> None:
        ...
        response_sender.send(
            pb_utils.InferenceResponse(), flags=pb_utils.TRITONSERVER_RESPONSE_COMPLETE_FINAL
        )
        ...
        with self.lock:
            ...
            for (
                ...
                self.req_id_to_request_data[req_id] = RequestData(
                    ...
                    batch_index,
                    len(batch_indices),
                    executor_request.sampling_config.num_return_sequences,
                    0,
                    0,
                ...
                input_len = (
                    len(executor_request.input_token_ids)
                    if executor_request.input_token_ids is not None
                    else 0
                )
                self.req_id_to_request_data[req_id].num_input_tokens += input_len
                # This checks both request level and instance config level
                if (
                    not executor_request.output_config.exclude_input_from_output
                    and not executor_request.streaming
                ):
                    self.req_id_to_request_data[req_id].num_output_tokens -= (
                        self.req_id_to_request_data[req_id].num_input_tokens
                        * executor_request.sampling_config.beam_width
                    )
                if triton_user_id is not None and triton_user_id != "":
        ...
        while self.running:
            for response in self.executor.await_responses(
                ...
                triton_response, is_final, output_length = convert_response(
                    response,
                    request_data.batch_index,
                    request_data.batch_size,
                    request_data.num_return_sequences,
                    self.logits_dtype,
                )
                with self.lock:
                    self.req_id_to_request_data[req_id].num_output_tokens += output_length
                triton_request_final = False
                if is_final:
                    with self.lock:
                        # Check if all executor requests part of that triton request are finished
                        self.triton_req_id_to_req_ids[request_data.triton_req_id].remove(req_id)
                        if len(self.triton_req_id_to_req_ids[request_data.triton_req_id]) == 0:
                            pb_utils.Logger.log_info(
                                f"DELETING Req id {req_id}, triton_req_id {request_data.triton_req_id} "
                            )
                            triton_request_final = True
                            del self.triton_req_id_to_req_ids[request_data.triton_req_id]
                            if (
                                request_data.triton_user_id is not None
                                and request_data.triton_user_id != ""
                            ):
                                del self.triton_user_id_to_req_ids[request_data.triton_user_id]
                ...
                request_data.response_sender.send(
                    triton_response,
                    flags=pb_utils.TRITONSERVER_RESPONSE_COMPLETE_FINAL
                    if triton_request_final
                    else 0,
                )

    def cancellation_loop(self) -> None:
    ...
    def update_metrics_per_request(self, req_id) -> None:
    ...
    def get_composite_metric_map(self, stat):
        def get_metric(metric_name, family_stats=None):
            if family_stats is None:
                ...
                if stat.kv_cache_stats is not None and hasattr(stat.kv_cache_stats, metric_name):
                    return getattr(stat.kv_cache_stats, metric_name)
                if stat.static_batching_stats is not None and hasattr(
                    stat.static_batching_stats, metric_name
                ):
                    return getattr(stat.static_batching_stats, metric_name)
                if stat.inflight_batching_stats is not None and hasattr(
        ...
        if max_blocks is not None and used_blocks is not None:
            composite_metrics["fraction_used_blocks"] = (
                0.0 if max_blocks <= 0 else used_blocks / max_blocks
            )
        else:
            pb_utils.Logger.log_warn(
                "fraction_used_blocks is missing one or more constituent metric."
            )
        ...
        if active_requests is not None and scheduled_requests is not None:
            ...
            pb_utils.Logger.log_warn(
                "num_waiting_requests is missing one or more constituent metric."
            )
            ...
            for stat in self.executor.get_latest_iteration_stats():
                try:
                    ...
                    for key, metric in self.all_metrics.items():
                        ...
                        elif stat.kv_cache_stats is not None and hasattr(stat.kv_cache_stats, key):
